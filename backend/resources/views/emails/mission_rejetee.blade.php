...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .hdr{background:linear-gradient(135deg,#C0392B 0%,#003DA5 100%);padding:36px 32px;text-align:center}
    ...
    .reject-banner{background:#FEF3F2;border-left:4px solid #C0392B;border-radius:8px;padding:16px 20px;margin-bottom:24px}
    .reject-banner-title{font-size:13px;font-weight:700;color:#C0392B;margin-bottom:6px}
    .reject-banner-text{font-size:14px;color:#4A1010;line-height:1.5}
...
<div class="wrap">
  <div class="card">
    <div class="hdr">
      ...
      <h1>Mission refusée</h1>
      <p>AT Réservations — Algérie Télécom</p>
    </div>
    <div class="body">
      <p class="greeting">Bonjour {{ $nomComplet ?: 'Collaborateur' }},</p>
      <p class="intro">Nous vous informons que votre demande de mission a été examinée et <strong>refusée</strong> par votre directeur. Veuillez consulter le motif ci-dessous.</p>

      @if($motif)
      <div class="reject-banner">
        <div class="reject-banner-title">Motif du refus :</div>
        <div class="reject-banner-text">{{ $motif }}</div>
      </div>
      @endif
      ...
      <a href="{{ $appUrl }}/missions/{{ $mission->id }}" class="btn">Voir les détails →</a>

      <p class="note">Vous pouvez soumettre une nouvelle demande de mission modifiée ou contacter votre directeur pour plus d'informations.</p>
    </div>
